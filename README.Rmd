...
3.  Install CGEI:
    ...
    -   Install `CGEI`
        ...
        `CGEI` is still in very active development. Therefore, the package is also not on CRAN yet. You can install the latest version of `CGEI` from GitHub with:\
...
theme_set(theme_bw())
g1 <- CGEI::visualizeWeights(x = 300, m = 0.5, b = 8, plot_engine = "ggplot2", mode = "logit")
g2 <- CGEI::visualizeWeights(x = 300, m = 1, b = 6, plot_engine = "ggplot2", mode = "exponential")
...
plot(crop(DSM, vs[[1]], mask = TRUE)); plot(observer, add = TRUE, pch = 19, col = "black")
plot(vs[[1]]); plot(observer, add = TRUE, pch = 19, col = "black")
...
The output of \~0.61 indicates, that \~61% of the visible area, within a 200 meters radius, is covered by greenspace.

#### 2. Road Network

We also provide sample data of a SF LINESTRING feature, representing a road network. This feature represents roads and paths, that can be reached by walking within 5 minutes from our observer location.
```{r message=FALSE, warning=FALSE}
...
Compute the VGVI along the line feature. As the resolution of our DSM is 5 meters, points along the line feature will be generated every 5 meters, too.
```{r message=FALSE, warning=FALSE}
...
The plot below shows the VGVI along the line feature. Yellow has a high VGVI, while blue has a low VGVI. The VGVI is highest in parks and lowest in urban center in the east.
```{r fig.height=6, fig.width=7, message=FALSE, warning=FALSE, echo=FALSE}
slope <- terrain(DSM, "slope", unit="radians")
aspect <- terrain(DSM, "aspect", unit="radians")
hill <- shade(slope, aspect, 40, 270)

plot(hill, col=grey(0:100/100), legend=FALSE)
plot(DSM, col=terrain.colors(100, alpha = 0.35), add=TRUE, legend=FALSE)
plot(vgvi_sf, add=TRUE, pch=16, cex = 0.5, col="white")
plot(vgvi_sf, add=TRUE, pch=16, cex = 0.4, fill=hcl.colors(nrow(vgvi_sf), alpha = 0.8, rev = TRUE))
```
